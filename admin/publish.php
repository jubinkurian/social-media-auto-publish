...
// 					$tw_max_len=$tw_max_len-24;
		...
		if($image_found==1 && $post_twitter_image_permission==1 && $xyz_smap_tw_app_sel_mode==0)
            ...
            /* api_exceed_err:
			if($api_exceed_err==1){
			    $tw_publish_status_insert= serialize("<span style=\"color:red\"> Daily API count limit exceeded,only ".$remaining_tw_api_count." api calls left.</span>");//1;
			}
			 */
...
// 			$caption_li=xyz_smap_string_limit($caption, 200);
		...
		elseif ($xyz_smap_ln_api_permission==2){
			...
			elseif ($ln_posting_method==2)//link share
			{
				...
				if (!empty($attachmenturl))
				{
				$thumbnail_arr=array('url'=>$attachmenturl);
				    $media_array=array( 'status'=> 'READY','description'=>array('text'=>$description_li),'originalUrl'=>$link,'title'=>$ln_title,'thumbnails'=>array($thumbnail_arr));
				}
				else
